...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pending = await getD1().prepare("SELECT id FROM deployments WHERE project_id = ? AND executor_job_id IS NOT NULL ORDER BY created_at DESC LIMIT 5").bind(id).all<{ id: string }>();
