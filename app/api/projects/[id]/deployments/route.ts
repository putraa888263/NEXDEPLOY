...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const results = await getD1().prepare("SELECT id, status, archive_name AS archiveName, executor, error, created_at AS createdAt, started_at AS startedAt, finished_at AS finishedAt FROM deployments WHERE project_id = ? ORDER BY created_at DESC LIMIT 20").bind(id).all();
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await getD1().batch([
    getD1().prepare("INSERT INTO deployments (id, project_id, status, requested_by, archive_key, archive_name, executor, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(deployment.id, id, deployment.status, user.id, project.archive_key, project.archive_name, deployment.executor, deployment.createdAt),
